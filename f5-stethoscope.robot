...
#             ${login_output}    SSHLibrary.Login with public key    username=${bigip_username}    keyfile=${bigip_ssh_identity_file}    delay=10 seconds
...
#                 SSHLibrary.Open Connection    ${bigip_host}    port=${bigip_ssh_port}
#                 # Log in and note the returned prompt
#                 ${login_output}   SSHLibrary.Log In    ${bigip_username}   ${bigip_password}    delay=10 seconds
#                 # Verify that the prompt includes (tmos)# for tmsh or the default bash prompt
#                 Should Contain Any    ${login_output}   (tmos)#    ] ~ #
#             EXCEPT
