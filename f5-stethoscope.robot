...
Retrieve Top 20 Directories and Files by Size on Disk
    ...
    ${top_20_directories}    Run BASH Command on BIG-IP    bigip_host=${bigip_host}   bigip_username=${bigip_username}   bigip_password=${bigip_password}    command=bash -c "find / -printf '%h\n' | sort | uniq -c | sort -k 1 -nr | head -20"
    Log to console    ${top_20_directories.json()}
    ${top_20_files}    Run BASH Command on BIG-IP    bigip_host=${bigip_host}   bigip_username=${bigip_username}   bigip_password=${bigip_password}    command=bash -c "find / -type f -exec du {} \; | sort -rn | head -20"
